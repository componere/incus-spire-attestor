...
const (
	...
	testConfigKeyPrefix = "user.spire.attestor.nonce."
	validConfigKeyHex   = "0123456789abcdef0123456789abcdef"
)

func validConfigKey() string {
	return testConfigKeyPrefix + validConfigKeyHex
}
...
func TestNewConfigKeyRejectsInvalidGrammar(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "empty", input: ""},
		{name: "prefix only", input: testConfigKeyPrefix},
		{name: "uppercase hexadecimal", input: testConfigKeyPrefix + "0123456789ABCDEF0123456789ABCDEF"},
		{name: "mixed-case hexadecimal", input: testConfigKeyPrefix + "0123456789abcdef0123456789ABCDEf"},
		{name: "31 hex characters", input: testConfigKeyPrefix + "0123456789abcdef0123456789abcde"},
		{name: "33 hex characters", input: testConfigKeyPrefix + validConfigKeyHex + "0"},
		{name: "extra trailing segment", input: validConfigKey() + ".extra"},
		{name: "slash in key", input: validConfigKey() + "/extra"},
		{name: "slash inside hex", input: testConfigKeyPrefix + "0123456789abcdef/123456789abcdef"},
		{name: "query string", input: validConfigKey() + "?x=1"},
		{name: "fragment", input: validConfigKey() + "#frag"},
		{name: "percent-encoded dot", input: testConfigKeyPrefix + "0123456789abcdef%2e23456789abcdef"},
		{name: "percent-encoded hex character", input: testConfigKeyPrefix + "0123456789abcdef%610123456789abcd"},
		...
		{
			name:  "reserved prefix as a longer user key",
			input: "user.spire.attestor.nonce." + validConfigKeyHex + ".more",
		},
...
func TestNewConfigKeyFromAttemptIDEmitsExactPrefixAndLowercaseHex(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			want := ConfigKey(testConfigKeyPrefix + hex.EncodeToString(tt.id[:]))
			assert.Equal(t, want, got, "attempt ID must encode as prefix plus 32 lowercase hex characters")
			assert.True(t, strings.HasPrefix(string(got), testConfigKeyPrefix))
			assert.Len(t, strings.TrimPrefix(string(got), testConfigKeyPrefix), 32)
			assert.Equal(t, strings.ToLower(string(got)), string(got), "hex suffix must be lowercase")

			roundTripped, err := NewConfigKey(string(got))
